...
export class PoolKeys {
    ...
    static connection = new Connection(pub, "confirmed");
    ...
    static async fetchPoolKeyInfo(baseMint: PublicKey, quoteMint: PublicKey): Promise<LiquidityPoolKeysV4> {
        const marketId = await this.fetchMarketId(this.connection, baseMint, quoteMint, 'finalized')
        const marketInfo = await this.fetchMarketInfo(marketId);
        const baseMintInfo = await this.connection.getParsedAccountInfo(baseMint) as MintInfo;
        ...
        const lpMintInfo = await this.connection.getParsedAccountInfo(V4PoolInfo.poolInfo.lpMint) as MintInfo;
